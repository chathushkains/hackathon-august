...
const OrdersChart = ({ data }) => {
  ...
  const minValue = Math.min(...data.map(item => item.orders))
  ...
  return (
    <div className="h-80 p-6 flex flex-col">
      <div className="flex-1 flex items-end justify-between space-x-3">
        {data.map((item, index) => {
          const height = range > 0 ? ((item.orders - minValue) / range) * 100 : 50
          return (
            <div key={index} className="flex-1 flex flex-col items-center">
              <div className="relative group w-full">
                <div 
                  className="bg-gradient-to-t from-blue-500 to-blue-400 rounded-t-lg transition-all duration-300 hover:from-blue-600 hover:to-blue-500 shadow-md hover:shadow-lg"
